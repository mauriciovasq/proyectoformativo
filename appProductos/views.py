...
def verProductosCategoria(request, idCategoria):
    ...
    listaProductos = Product.objects.filter(categoria= idCat)
...
def verProducto(request, idProd, msj = None):
    #Consultar 
    idProd = int(idProd)
    regProducto = Product.objects.get(id= idProd)
    #ensamblar context
    context = {
...
def verTodos(request):
    ...
    listaProductos = Product.objects.all().values('id', 'nombre', 'categoria__id')
    #armar el context
    context = {
...
def agregarCarro(request, idProd):
    ...
    existe = Product.objects.filter(id=idProd).exists()
    if existe:
        regProducto = Product.objects.get(id=idProd)
